 ...
 *   WE MADE CHANGES TO THIS ONE
 */

public class Board extends JPanel implements KeyListener
{		
  private static final int X_DIM = 60; //we changed the size of the grids
  ...
  //keyboard presses boolean variable
  boolean key1 = false; 
  boolean key2 = false;
  boolean key3 = false;
  boolean key4 = false;
  
  //window (We made this global)
  JFrame f;

  
  // Grid colours
  private static final Color GRID_COLOR_A = new Color(0,0,0); //black
  private static final Color GRID_COLOR_B = new Color(0,100,0); //dark green
  ...
  {
    super( true );
    f = new JFrame ("Space Invaders - Milena Mijuskovic and Selena Zhang");
  ...
  private void paintText(Graphics g)
  {
    ...
    g.setFont(new Font("OCR A Extended", Font.PLAIN, 18)); //we changed the font
  ...
  private void paintGrid(Graphics g)
  {
    for (int i = 0; i < this.grid.length; i++)
    {
      ...
      {    
        //This paints the grid black 
        g.setColor(GRID_COLOR_A);
        if(j == this.grid[i].length-1) //this paints the last row dark green
  ...
  public int getRows()
  {
	...
	@Override
	public void keyTyped(KeyEvent e) //If a key is pressed, the boolean for that key is turned to true
	{
		// TODO Auto-generated method stub
		if(e.getKeyChar()=='1')
		{
